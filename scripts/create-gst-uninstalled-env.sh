...
check_distro() {
	if [ -f /etc/lsb-release ]; then
		...
		if [ "$DISTRO_ID" = "Raspbian" ]; then
			GST_PLUGINS_BAD_CONFIGURE_FLAGS="--disable-gtk-doc --disable-opengl --enable-gles2 --enable-egl --disable-glx --disable-x11 --disable-wayland --enable-dispmanx  --with-gles2-module-name=/opt/vc/lib/libGLESv2.so --with-egl-module-name=/opt/vc/lib/libEGL.so"
